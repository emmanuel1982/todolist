...
addButton.addEventListener('click', function () {
    ...
    // Your sort event listener is inside your add event listener. Let's remove it to the outside because it is independant of the items. You want to create the sort event listener only once and not each time the add button gets hit. 
    sort.addEventListener('change', () => {
        ...
        if (sortValue.options[sortValue.selectedIndex].text == 'Ascending') {
            ...
            // And you want to so the same in descending as well
            listItem.remove();
            asc = todoArray.sort();
            ...
            for (let i = 0; i < asc.length; i++) {
                ...
                // So at this point you may have noticed that you are basically writing the same code several times i.e. creating those elements in the add event listener, in sort scending and sort descending. You want to create a function that does that and just call the function each time. 
